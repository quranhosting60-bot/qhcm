...
export default function CallsPage() {
  ...
  const [form, setForm] = useState({ leadId: '', callType: 'whatsapp' as 'whatsapp' | 'viki', outcome: 'interested', duration: '', notes: '', date: today })

  const loadData = async () => {
    setLoading(true)
    try {
      let callParams = 'select=*&order=created_at.desc'
      if (viewMode === 'daily') callParams += `&call_date=eq.${selectedDate}`
      else { const m = selectedDate.slice(0, 7); callParams += `&call_date=gte.${m}-01&call_date=lte.${m}-31` }
      ...
      setCalls(callData || []); setLeads(leadData || [])
  ...
  const getLeadDisplay = (leadId: string) => {
    ...
    return idx >= 0 ? `Lead ${idx + 1} — ${leads[idx].phone}` : 'Unknown Lead'
  }
  ...
  const handleSave = async () => {
    ...
    try {
      await apiFetch('call_logs', 'POST', 'select=*', {
        lead_id: form.leadId, agent_id: user.id, agent_name: user.name,
        call_type: form.callType, call_date: form.date,
        duration: form.duration ? Number(form.duration) : null,
        outcome: form.outcome, notes: form.notes || null,
      })
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <table className="crm-table">
              <thead>
                <tr>
                  {['Lead', 'Agent', 'Type', 'Date', 'Duration', 'Outcome', 'Notes'].map(h => <th key={h}>{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {calls.length === 0 && <tr><td colSpan={7} className="text-center py-12 text-slate-500">No calls found for this period</td></tr>}
                {calls.map(call => (
                  <tr key={call.id}>
                    <td><span className="font-bold text-emerald-400">{getLeadDisplay(call.lead_id)}</span></td>
                    <td><span className="text-slate-300">{call.agent_name}</span></td>
                    <td>
                    ...
                    <td><span className={outcomeBadge[call.outcome] || 'badge badge-gray'}>{OUTCOMES.find(o => o.value === call.outcome)?.label || call.outcome}</span></td>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <select className="crm-select" value={form.leadId} onChange={e => setForm({ ...form, leadId: e.target.value })}>
                  ...
                  {leads.map((l, i) => <option key={l.id} value={l.id}>Lead {i + 1} — {l.phone}</option>)}
                </select>
              </div>
